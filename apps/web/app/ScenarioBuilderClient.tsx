...
const customTemplateFallback: ScenarioTemplate = {
  id: 'custom-scenario',
  label: 'Custom scenario',
  description: 'Provide scenario parameters manually when no presets are available.',
  defaults: {
    initialBalance: 0,
    riskPerTrade: 0,
    maxPositions: 0,
  ...
  notes: [
    'Fill in each field to craft bespoke scenario parameters.',
    'Adjust balances, risk, and timeframe until warnings clear.',
  ],
}
...
type ScenarioBuilderClientProps = {
  templates: ScenarioTemplate[]
}

export default function ScenarioBuilderClient({ templates }: ScenarioBuilderClientProps) {
  const hasTemplates = templates.length > 0
  const [templateId, setTemplateId] = useState<string>(hasTemplates ? templates[0].id : '')
  const [draft, setDraft] = useState<ScenarioDraft>(() =>
    hasTemplates ? toDraft(templates[0].defaults) : createEmptyDraft(),
  )
  ...
  const selectedTemplate = useMemo(() => {
    ...
    return customTemplateFallback
  }, [hasTemplates, templateId, templates])
  ...
  const resetTemplate = () => {
    ...
    setDraft(template ? toDraft(template.defaults) : createEmptyDraft())
  }
