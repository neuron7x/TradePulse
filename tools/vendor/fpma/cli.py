...
def main():
    ...
    p_init = sub.add_parser("init", help="Initialize a new fractal monorepo (Bazel-based by default).")
    p_init.add_argument("--name", required=True, help="Repository name (directory will be created if missing).")
    ...
    p_init.add_argument("--no-bazel", action="store_true", help="Skip Bazel skeleton (not recommended).")
    ...
    p_fu.add_argument("--lang", choices=["python","node"], default="python", help="Implementation language.")
    p_fu.add_argument("--with-openapi", action="store_true", help="Include OpenAPI 3.1 contract in api/.")
    ...
    p_check = sub.add_parser("check", help="Run algorithmic checks (invariants, cycles, complexity).")
    p_check.add_argument("--path", required=True, help="Path to repo root.")
    p_check.add_argument("--max-cyc", type=int, default=10, help="Max cyclomatic complexity per function.")
    p_check.add_argument("--fail-on", choices=["any","cycles","imports","complexity"], default="any", help="Failing policy.")
    p_check.add_argument("--json", action="store_true", help="Emit JSON report.")

    args = parser.parse_args()

    if args.cmd == "init":
        init_repo(args.name, license_id=args.license, include_ci=args.ci, include_bazel=not args.no_bazel)
        print(f"Initialized FPM-A repo at {args.name}")
        return 0

    if args.cmd == "add-fu":
        add_fu(Path(args.path), args.name, args.domain, args.lang, with_openapi=args.with_openapi)
    ...
    if args.cmd == "check":
        ...
        if args.json:
            ...
            print(f"  functions_scanned={report['complexity']['functions_scanned']} max={report['complexity']['max']} avg={report['complexity']['avg']:.2f}")
        fail = False
        if args.fail_on == "cycles" and cycles:
            ...
            fail = True
        elif args.fail_on == "any" and (cycles or inv_violations or report['complexity']['max'] > args.max_cyc):
            fail = True
