    ...
    class V(ast.NodeVisitor):
        ...
        def visit_FunctionDef(self, node):
            ...
            for n in ast.walk(node):
                if isinstance(n, (ast.If, ast.For, ast.While, ast.With, ast.BoolOp, ast.Try, ast.ExceptHandler, ast.Match)):
                    score += 1
...
def scan_complexity(root:Path):
    ...
    return {"functions_scanned":len(scores), "max":max(scores), "avg":statistics.mean(scores)}
