...
def _prepare_dataframe(df: pd.DataFrame, window: int) -> tuple[pd.DataFrame | None, list[str], list[str]]:
    ...
    if len(working) < window:
        errors.append(
            f"Dataset length ({len(working)}) is smaller than the configured lookback window ({window})."
        )
...
with st.sidebar:
    ...
    delta = st.number_input("Ricci delta", min_value=0.0001, max_value=0.1, value=0.005, step=0.0005, format="%0.4f")
    use_gpu = st.toggle("Use GPU acceleration", value=False, help="Requires CuPy support at runtime")
    ...
    try:
        ...
        if errors:
            ...
            with tracer.start_as_current_span(
                ...
                span.set_attributes({"metrics.phase": metrics["phase"], "metrics.gpu_used": metrics["gpu_used"]})

            if use_gpu and not metrics["gpu_used"]:
                st.warning("GPU acceleration was requested but the CPU implementation was used instead.")
