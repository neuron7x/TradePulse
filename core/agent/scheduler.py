...
class StrategyScheduler:
    ...
    def run_pending(self) -> Dict[str, list[EvaluationResult]]:
        ...
        futures: list[tuple[_JobState, Future[tuple[EvaluationResult, ...] | None]]] = []
        for state in sorted(due_states, key=lambda item: item.next_run):
            future = self._executor.submit(self._execute_job, state)
    ...
    def _execute_job(self, state: _JobState) -> tuple[EvaluationResult, ...] | None:
        ...
        self._handle_success(state, evaluations)
        return evaluations
