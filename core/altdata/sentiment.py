...
class SentimentFeatureBuilder:
    ...
    def _prepare(self, signals: Iterable[SentimentSignal]) -> pd.DataFrame:
        ...
        if not rows:
            return pd.DataFrame(columns=["timestamp", "source", "score", "volume"]).set_index("timestamp")
    ...
    def latest(self, signals: Iterable[SentimentSignal]) -> dict[str, float]:
        ...
        return {"sentiment_vwap": vwap, "sentiment_momentum": momentum, "sources": float(recent["source"].nunique())}
