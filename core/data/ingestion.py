...
class DataIngestor(DataIngestionService):
    ...
    def historical_csv(
        ...
        *,
        ...
        with metrics.measure_data_ingestion("csv", symbol) as ctx:
            ctx.update(
                {
                    "venue": venue,
    ...
    def binance_ws(self, symbol: str, on_tick: Callable[[Ticker], None], *, interval: str = "1m") -> object:
        ...
        def _callback(message: dict) -> None:
            with metrics.measure_data_ingestion("binance", symbol) as ctx:
                ctx.update({"venue": "BINANCE", "interval": interval})
                event_type = message.get("e")
                if event_type is not None:
                    ctx["event_type"] = str(event_type)
                ...
                try:
                    ...
                    ctx["status"] = "parse_error"
                    logger.warning("Failed to parse websocket payload: %s", exc)
                    return
                on_tick(tick)
                metrics.record_tick_processed("binance", symbol)
