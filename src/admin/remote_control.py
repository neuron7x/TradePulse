...
def _resolve_registry(metric_kwargs: Mapping[str, Any]) -> Any | None:
    """Return the registry that should hold the metric if available."""

    if "registry" in metric_kwargs:
        return metric_kwargs["registry"]
    return REGISTRY


def _get_or_create_counter(*, name: str, documentation: str, metric_kwargs: Mapping[str, Any]) -> Counter:
    """Create or fetch a Prometheus counter, avoiding duplicate registrations."""

    registry = _resolve_registry(metric_kwargs)
    ...
    try:
        return Counter(name, documentation, **metric_kwargs)
    except ValueError as exc:  # pragma: no cover - defensive path
        if registry is not None and "Duplicated timeseries" in str(exc):
            existing = getattr(registry, "_names_to_collectors", {}).get(name)
            if isinstance(existing, Counter):
                return existing
        raise
...
def _get_or_create_histogram(*, name: str, documentation: str, metric_kwargs: Mapping[str, Any]) -> Histogram:
    """Create or fetch a Prometheus histogram, avoiding duplicate registrations."""
    ...
    try:
        return Histogram(name, documentation, **metric_kwargs)
    except ValueError as exc:  # pragma: no cover - defensive path
        if registry is not None and "Duplicated timeseries" in str(exc):
            existing = getattr(registry, "_names_to_collectors", {}).get(name)
            if isinstance(existing, Histogram):
...
class RemoteControlMetrics:
    ...
    def __post_init__(self) -> None:
        ...
        self.engagements = _get_or_create_counter(
            name="tradepulse_admin_kill_switch_total",
            documentation="Total kill-switch engagement attempts",
            metric_kwargs=metric_kwargs,
        )
        self.rate_limited = _get_or_create_counter(
            name="tradepulse_admin_kill_switch_rate_limited_total",
            documentation="Total kill-switch requests rejected by rate limiting",
            metric_kwargs=metric_kwargs,
        )
        self.errors = _get_or_create_counter(
            name="tradepulse_admin_kill_switch_errors_total",
            documentation="Total kill-switch attempts that resulted in errors",
            metric_kwargs=metric_kwargs,
        )
        self.latency_seconds = _get_or_create_histogram(
            name="tradepulse_admin_kill_switch_latency_seconds",
            documentation="Time spent executing kill-switch operations",
            metric_kwargs=metric_kwargs,
        )
...
def _extract_idempotency_key(request: Request) -> str:
    """Return the request idempotency key, validating the required header."""
...
def _resolve_idempotency_key(
    ...
    *,
    ...
    """Return a validated or derived idempotency key for the request."""

    try:
        return _extract_idempotency_key(request)
    except HTTPException as exc:
        if require_header:
            raise exc
        reason = payload.reason.strip()
        raw = f"{subject}:{request.method}:{request.url.path}:{reason}".encode("utf-8")
...
def create_remote_control_router(
    ...
    *,
    ...
    require_idempotency_header: bool | None = None,
) -> APIRouter:
    ...
    enforce_idempotency_header = (
        require_idempotency_header
        if require_idempotency_header is not None
        else idempotency_cache is not None
    )
    ...
    async def engage_kill_switch(
        ...
        idempotency_key = _resolve_idempotency_key(
            ...
            payload=payload,
            require_header=enforce_idempotency_header,
        )
