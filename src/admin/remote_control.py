    ...
    async def snapshot(self) -> AdminRateLimiterSnapshot:
        ...
        async with self._lock:
            ...
            for identifier, bucket in list(self._records.items()):
                # Robust cleanup: remove entries that are either clearly invalid (<= 0.0)
                # or older than the configured interval according to the same clock.
                while bucket and (
                    bucket[0] <= 0.0 or now - bucket[0] >= self._interval
                ):
                    bucket.popleft()
                if bucket:
                    cleaned[identifier] = deque(bucket)
                else:
                    # remove empty buckets from backing store
