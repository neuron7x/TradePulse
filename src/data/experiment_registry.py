...
class ArtifactRecord(BaseModel):
    ...
    uri: str = Field(..., description="URI that locates the artifact (can be local path or remote storage URI).")
    kind: str = Field(
        default="generic",
...
class ExperimentRunRecord(BaseModel):
    ...
    params: dict[str, Any] = Field(default_factory=dict, description="Hyperparameters used during training.")
    metrics: dict[str, float] = Field(default_factory=dict, description="Evaluation metrics captured at the end of the run.")
    artifacts: list[ArtifactRecord] = Field(
        default_factory=list,
        description="Artifacts generated by the run.",
    )
    tags: set[str] = Field(default_factory=set, description="Optional set of tags to help discover runs.")
...
def _diff_params(previous: Mapping[str, Any], current: Mapping[str, Any]) -> dict[str, Mapping[str, Any]]:
...
def _metrics_delta(previous: Mapping[str, float], current: Mapping[str, float]) -> dict[str, Mapping[str, float | None]]:
...
class ExperimentRegistry:
    ...
    def reproducibility_manifest(self, run_id: str, *, experiment_name: str | None = None) -> dict[str, Any]:
