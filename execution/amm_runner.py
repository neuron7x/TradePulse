...
class AMMRunner:
    def __init__(self, symbol: str, tf: str, cfg: AMMConfig | None = None, metrics_port: int = 9095):
    ...
    async def on_tick(self, x: float, R: float, kappa: float, H: float | None = None) -> dict:
        ...
        publish_metrics(self.symbol, self.tf, out, k=self.amm.gain, theta=self.amm.threshold, q_hi=None)
        return out
