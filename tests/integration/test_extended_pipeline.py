...
class TestEndToEndPipeline:
    ...
    def test_strategy_to_backtest_pipeline(self) -> None:
        ...
        strategy = Strategy(name="test", params={"lookback": 10, "threshold": 0.3})

        # Use strategy to create signal function
        def signal_from_strategy(p: np.ndarray) -> np.ndarray:
            # Simple momentum signal
            returns = np.diff(p, prepend=p[0])
            threshold = float(strategy.params.get("threshold", 0.0))
            signals = np.where(returns > threshold, 1.0, -1.0)
            return signals
