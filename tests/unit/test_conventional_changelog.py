...
def test_render_changelog_groups_by_category() -> None:
    commits = [
        CommitInfo(sha="a" * 40, type="feat", scope="ingest", subject="Add kafka source", breaking=False, notes=[]),
        CommitInfo(sha="b" * 40, type="fix", scope=None, subject="Handle reconnects", breaking=False, notes=[]),
        CommitInfo(sha="c" * 40, type="docs", scope=None, subject="Document release process", breaking=False, notes=[]),
    ]
