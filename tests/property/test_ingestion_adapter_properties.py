        ...
        st.fixed_dictionaries(
            {
                ...
                "volume": st.floats(min_value=0.0, max_value=10_000_000.0, allow_nan=False, allow_infinity=False),
            }
        ),
        min_size=1,
        max_size=25,
...
def test_csv_adapter_preserves_numeric_precision(rows: list[dict[str, float]], tmp_path: Path) -> None:
    ...
    ticks = asyncio.run(
        adapter.fetch(path=csv_path, symbol="BTCUSD", venue="BINANCE", timestamp_field="ts", price_field="price", volume_field="volume")
    )
    ...
    for original, parsed in zip(rows, ticks, strict=False):
        ...
        assert parsed.ts == pytest.approx(float(original["ts"]), rel=FLOAT_REL_TOL, abs=TIMESTAMP_ABS_TOL)
        assert float(parsed.price) == pytest.approx(float(original["price"]), rel=FLOAT_REL_TOL, abs=FLOAT_ABS_TOL)
        assert float(parsed.volume) == pytest.approx(float(original["volume"]), rel=FLOAT_REL_TOL, abs=FLOAT_ABS_TOL)
...
def test_parquet_adapter_matches_source_values(rows: list[dict[str, float]], tmp_path: Path) -> None:
    ...
    ticks = asyncio.run(
        adapter.fetch(path=parquet_path, symbol="ETHUSD", venue="COINBASE", timestamp_field="ts", price_field="price", volume_field="volume")
    )
