...
import numpy as np
from core.neuro.calibration import calibrate_random, CalibConfig
...
def _toy_series(n=2000, seed=0):
    ...
    R = np.full(n, 0.55, dtype=np.float32); R[1000:] = 0.7
    kappa = np.full(n, 0.1, dtype=np.float32); kappa[1000:] = -0.1
    return x, R, kappa
