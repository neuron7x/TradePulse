...
def test_inventory_latency_monitor_report_export(tmp_path) -> None:
    monitor = InventoryLatencyMonitor(window=16)
    monitor.record_sample(
        InventoryLatencyStage.SNAPSHOT, timedelta(milliseconds=2)
    )
    monitor.record_sample(
        InventoryLatencyStage.SNAPSHOT, timedelta(milliseconds=3)
    )
    monitor.record_sample(
        InventoryLatencyStage.ALLOCATION, timedelta(milliseconds=1)
    )
    path = InventoryManager(  # use helper to export via manager interface
        _build_ledger(),
        {"BTCUSDT": ("BTC", "USDT")},
        latency_monitor=monitor,
    ).export_latency_report(directory=tmp_path, filename="latency.json")
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert "snapshot" in payload["stages"]
    assert payload["stages"]["snapshot"]["samples"] == 2
    assert payload["stages"]["allocation"]["samples"] == 1


def test_inventory_manager_enforces_latency_budget() -> None:
    ledger = _build_ledger()
    ledger.set_balance(
        "EX1",
        "BTCUSDT",
    ...
    strict_threshold = LatencyThreshold(
        p95=timedelta(microseconds=1),
        hard_limit=timedelta(microseconds=1),
        min_samples=1,
    )
    tolerant_threshold = LatencyThreshold(
        p95=timedelta(milliseconds=50),
        hard_limit=timedelta(milliseconds=100),
        min_samples=1,
    )
    budget = InventoryLatencyBudget(
        snapshot=strict_threshold,
        target_computation=tolerant_threshold,
        rebalance=tolerant_threshold,
        allocation=tolerant_threshold,
    )
    manager = InventoryManager(
        ledger,
        {"BTCUSDT": ("BTC", "USDT")},
    ...
    with pytest.raises(InventoryError, match="snapshot latency"):
        manager.propose_rebalance("BTCUSDT", targets)
