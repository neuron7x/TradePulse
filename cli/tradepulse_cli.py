...
def ingest(
    ...
    message = (
        f"[{command}] completed records={record_count} dest={cfg.destination} "
        f"sha256={digest}"
    )
    click.echo(message)
...
def backtest(
    ...
    message = f"[{command}] completed stats={stats_json} sha256={digest}"
    click.echo(message)
...
def optimize(
    ...
    message = (
        f"[{command}] completed trials={len(trials)} best_score={best_score} "
        f"sha256={digest}"
    )
    click.echo(message)
...
def exec(  # noqa: A001
    ...
    message = f"[{command}] completed latest_signal={latest} sha256={digest}"
    click.echo(message)
...
def report(
    ...
    message = f"[{command}] completed sections={len(cfg.inputs)} sha256={digest}"
    click.echo(message)
